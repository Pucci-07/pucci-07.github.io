...
Feroxbuster is a command-line tool designed to discover hidden or forgotten directories and files in web applications. Written in the Rust programming language, Feroxbuster stands out for its high performance and effective scanning capabilities. It is frequently used by security professionals, penetration testers, and web developers. This tool employs brute-force and wordlist attack methods to scan web servers.

### Features

- **High Performance:** Written in Rust, making it extremely fast and efficient.
- **Flexibility:** Allows customization of various parameters such as the number of threads per request, scanning depth, and timeout durations.
- **Versatility:** Feroxbuster not only performs directory and file scanning but also features different scanning modes, including recursive scanning and highlighting.

In a simple usage scenario, you can use Feroxbuster to scan for directories and files on a specific website. The following command performs a basic scanning operation:

### Help Menu

```auto
root💀hackerbox:~# feroxbuster --help
...
```auto
root💀hackerbox:~# feroxbuster -u 172.20.8.56 -w /root/Desktop/misc/SecLists/Discovery/Web-Content/directory-list-1.0.txt
...
Let's perform a file scan on a target system with the command below. We specify the wordlist with the
...
Use the following command to detect files with the
...
```auto
root💀hackerbox:~# feroxbuster -u 172.20.8.56 -w /root/Desktop/misc/SecLists/Discovery/Web-Content/common.txt -x pdf
